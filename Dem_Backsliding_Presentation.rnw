...
<<conjoint:data:analyses, echo=FALSE, comment=NA, warning=FALSE, message=FALSE, include = TRUE, cache=TRUE, eval=TRUE>>=
...
\center
\Sexpr{sumtable(dat,labels=labs, vars = vars,out='latex')}
\hyperlink{sum:table:slide}{\beamergotobutton{go back}}
\end{frame}
