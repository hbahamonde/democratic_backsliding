  ...
  \begin{enumerate}
    \item {\bf Social movements}:
      ...
      \begin{itemize}
        ...
        \item[{\color{red}\Pointinghand}] Introduce the case of {\bf violent protests}.
...
\begin{column}[T]{0.45\textwidth}
    ...
    \begin{itemize}
        ...
        \item {\only<4>{\leavevmode\color{Red}}{\bf Disengaged citizens}: did not vote in the last Parliamentary elections favor candidates proposing violent protests.}    
...
\begin{column}[T]{0.45\textwidth}
    ...
    \begin{itemize}
        ...
        \item {\only<2>{\leavevmode\color{Red}}(Hypothetical) Candidates {\bf favoring violent protests} are systematically {\bf rejected} by {\bf both} winners and losers.}

        \item {\only<4>{\leavevmode\color{Red}}{\bf Legacy effects}: results are driven by the legacies of the 2019-20 protests (\emph{Social Unrest}).}
