...
\begin{columns}[T] % The [T] option aligns column content at the top
    ...
    \begin{column}{0.5\textwidth} % Left column and width
        \begin{itemize}
            ...
            \item<4-> Unfortunately, the ``{\bf \color{red}losers' consent}'' {\bf \color{red}literature has not studied the propensity to support anti-systemic protests}.
        \end{itemize}
    \end{column}
  ...
  \begin{enumerate}
    \item {\bf Social movements}:
      ...
      \begin{itemize}
        ...
        \item[{\color{red}\Pointinghand}] Introduce the case of {\bf violent protests}.\pause
...
\begin{frame}[c]{Conjoint Experiment}
  ...
  \begin{itemize}
    ...
    \item We explored if {\bf hypothetical} (conjoint) {\bf candidates} who support violent protests receive different levels of support from {\bf actual election winners and losers} (participants).
...
\begin{frame}[c]{Discussion: More Questions Than Answers}
  \begin{itemize}
    ...
    \begin{itemize}
      \item {\bf Chile}: {\color{blue}Extreme-right} supporters (winners) are {\color{blue}less} likely to support violent protests.
      \item {\bf Estonia}: {\color{blue}Extreme-right} supporters (losers) are {\color{blue}more} likely to support violent protests.
    \end{itemize}
