...
\begin{frame}[c]{Theoretical Background}
  ...
  \item {\bf Institutional literature}: institutional setups affect differently the costs of losing an election \tiny{{\color{gray}(Lijphart, 2012)}}.
        ...
        \item[{\color{red}\Pointinghand}] Thus, in presidential systems ``\emph{losers lose more},'' having more incentives to oppose the government via {\bf violent protests}.
...
\begin{frame}[c]{Conjoint Experiment}
  ...
  \begin{itemize}
    ...
    \item We explored if {\bf hypothetical} (conjoint) {\bf candidates} who support violent protests receive different levels of support from {\bf actual winning and losing participants}.
...
\begin{column}[T]{0.45\textwidth}
    ...
    \begin{itemize}
        ...
        \item {\only<3>{\leavevmode\color{Red}}{\bf Losers' Effect}: largely driven by anti-migrant and anti-LGBT extreme right EKRE voters.}
