...
<<echo=FALSE, cache=FALSE, warning = FALSE, message = F, eval=TRUE>>=
knitr::read_chunk('../democratic_backsliding/Dem_Backsliding.r')
...
<<conjoint:prep, echo=FALSE, cache=FALSE, comment=NA, warning=FALSE, message=FALSE, include = TRUE, eval=TRUE>>=

@

<<conjoint:data:analyses, echo=FALSE, comment=NA, warning=FALSE, message=FALSE, include = TRUE, cache=FALSE, eval=FALSE>>=

@

<<conjoint:data:plot, echo=FALSE, comment=NA, warning=FALSE, message=FALSE, include = TRUE, cache=FALSE, eval=FALSE>>=

@

<<descriptive:data:plot, echo=FALSE, comment=NA, warning=FALSE, message=FALSE, include = TRUE, cache=FALSE, eval=FALSE>>=
...
\begin{frame}[c]{Democratic Backsliding}
  \begin{itemize}
    ...
    \item Estos retrocesos han sido estudiados en un sinn\'umero de casos.
        ...
        \begin{itemize}
          \item \textcite{Kaufman2019} explican que ``a transition to competitive authoritarianism in the {\bf United States} is unlikely, although not impossible.''

          \item Kreko and Zsolt (2018) estudian el caso de {\bf Hungria}.
          \item Bochsler and Andreas (2020) estudian el caso de {\bf Rusia}. 
...
\begin{frame}[c]{Nuestro Paper: ``El baile de los que sobran''}
  ...
  \begin{itemize}
    ...
    \item Hicimos un conjoin experiment (pre-registrado) en dos democracias recientes, Chile (y Estonia).
  ...
  \end{itemize}
      ...
      Encontramos que los votantes de {\bf Kast} (perdedores) \emph{no} son mas proclives que los votantes de {\bf Boric} (ganadores) a apoyar acciones no democraticas que pongan en peligro el \emph{status quo} (protestas antisist\'emicas). 
      \end{exampleblock}
...
\begin{frame}[c]{Democratic Backsliding}
  ...
  \begin{itemize}
    \item \textcite{Bermeo2016} ``{\bf introduced} the landmark essay on democratic backsliding'' {\scriptsize\parencite[p. 27]{Haggard2021ca}}.
    \item Democratic institutions, norms, and values are gradually eroded {\bf within} a political system.
    \item Erosion of civil liberties, undermining of free and fair elections, and the {\bf concentration of power in the executive branch}.
  \end{itemize}
...
Respondents who voted for the candidate/party that lost the the last election (Kast) would be more likely to choose a candidate who supports anti-systemic actions (protests) against the current government than respondents in the ``winning side'' (Boric). 
\end{exampleblock}
...
\begin{frame}[c]{Conjoint Experiment}
  ...
  \begin{itemize}
    ...
    \item La muestra es representativa a nivel pais ($n=\Sexpr{chile.sample.size}$).
...
<<sum:table, echo=FALSE, comment=NA, warning=FALSE, message=FALSE, include = T, cache=FALSE, eval=TRUE, results = "asis">>=
