...
<<conjoint:prep, echo=FALSE, comment=NA, warning=FALSE, message=FALSE, include = TRUE, eval=TRUE>>=

@

<<conjoint:data:analyses, echo=FALSE, comment=NA, warning=FALSE, message=FALSE, include = TRUE, cache=FALSE, eval=TRUE>>=

@

<<sum:table, echo=FALSE, comment=NA, warning=FALSE, message=FALSE, include = F, cache=FALSE, eval=TRUE, results = "asis">>=
...
Desafortunadamente, la major\'ia ha concentrado sus esfuerzos en c\'omo el ejecutivo agranda sus poderes. 
...
%\begin{alertblock}{Voters}
%Y aunque los votantes tambi\'en han sido estudiados (e.g., \cite{Svolik2019}), los incumbentes siempre son los protagonistas de estos retrocesos.
%\end{alertblock}
...
\begin{frame}[c]{Our paper}
  ...
  \begin{itemize}
    \item Unlike most research that concentrates on potential breaches of democratic values by the ``winners,'' we turn our attention to electoral ``losers.''
    
    \item We seek to understand if voters who sided with the losing candidate are more open to supporting anti-systemic actions against the incumbent.]

    \item We carried out a novel, fully pre-registered survey experiment in two new democracies, Chile (and Estonia), probing into the willingness of these ``losers'' to tolerate transgressions against democratic principles.
...
\center
Test \Sexpr{sumtable(dat,labels=labs, vars = vars,out='latex')}
\end{frame}
