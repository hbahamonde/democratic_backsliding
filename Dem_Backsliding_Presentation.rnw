...
\framesubtitle{Electoral Losses and Institutional Heterogeneities}
  ...
  \begin{itemize}
    \item We analyze the level of system support among electoral ``losers'' using data from a novel conjoint experiment in Chile (N=\Sexpr{chile.sample.size}) and Estonia (N=\Sexpr{estonia.sample.size}).
...
\begin{column}[T]{0.45\textwidth}
    ...
    \begin{itemize}
        ...
        \item {\only<3>{\leavevmode\color{Red}}{\bf Losers} show even {\bf stronger disapproval} of such candidates.}
...
\subsection{Exploratory Analysis (MM): Partisanship/Ideology}
